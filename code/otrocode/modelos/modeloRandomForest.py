...
for i, datos in enumerate(conjuntos_de_datos, 1):
    ...
    predicciones = forest.predict(df_nuevo)
    predicciones_probabilidades = forest.predict_proba(
        df_nuevo)[:, 1]  # Probabilidad de la clase positiva
    predicciones_binarias = (predicciones_probabilidades >= umbral).astype(int)

    print(
        f"\nResultados de Predicciones para Nuevas Parcelas (Random Forest - Conjunto {i}):")
    print(predicciones)
